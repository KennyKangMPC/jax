...
ShardingSpecAndAval = Tuple[Sequence[ShardingSpec], Sequence[ShapedArray]]

def _get_mesh_sharding_spec_and_avals(shardings, avals, is_global) -> ShardingSpecAndAval:
...
def _get_sharding_spec_and_avals(shardings, avals) -> ShardingSpecAndAval:
  ...
  if config.jax_parallel_functions_output_gda:
    assert isinstance(shardings[0], sharding.MeshPspecSharding)
    return _get_mesh_sharding_spec_and_avals(shardings, avals, is_global=True)
  elif config.jax_array:
    if isinstance(shardings[0], sharding.PmapSharding):
    ...
    assert isinstance(shardings[0], sharding.MeshPspecSharding)
    return _get_mesh_sharding_spec_and_avals(shardings, avals, is_global=False)


def global_avals_to_results_handler(
    global_out_avals: Sequence[ShapedArray], shardings):
